...
const RoleForm = (props) => {
    const {addRolesData, singleRole, editRole, permissionsArray, id, newArray} = props;
    ...
    useEffect(()=> {
        ...
        if (singleRole?.warehouses) {
            const selectedWarehouses = singleRole.warehouses.map(w => ({
                label: w.name,
                value: w.id,
            }));
            setWarehouseValue(selectedWarehouses);
        }
    }, [singleRole])
    ...
    const disabled = warehouseValue.length === 0 ? true : saveButtonEnable.length === 0 ? true : singleRole && singleRole.name === rolesValue.name && JSON.stringify(singleRole.permissions.map((item)=>item.id)) === JSON.stringify(saveButtonEnable);
    ...
    const handleValidation = () => {
        ...
        } else if (warehouseValue.length === 0) {
            errorss['warehouses'] = 'Please select at least one warehouse.';
        } else {
            isValid = true;
        }
        setErrors(errorss);
    ...
    const onWarehouseChange = (selectedOptions) => {
        setWarehouseValue(selectedOptions);
    };
    ...
    const onSubmit = (event, rolesValue) => {
        ...
        if (Valid) {
            ...
            rolesValue.warehouses = warehouseValue.map(w => w.value);
                        ...
                        <div>
                            <Form.Group className='mb-5 form-group'>
                                ...
                                <div className='d-flex col-md-12 flex-wrap'>
                                    ...
                                    <TabTitle title="Warehouses" />
                                    <div className="d-flex flex-wrap col-md-12">
                                    <h2 className='col-12'>Warehouses</h2>
                                        {newArray && newArray.map((warehouse, index) => {
                                            const isChecked = warehouseValue.some(w => w.value === warehouse.value);
                                            ...
                                            <div className="col-md-4" key={index}>
                                                ...
                                                <input
                                                    type="checkbox"
                                                    className="me-3 form-check-input cursor-pointer"
                                                    name={`warehouse-${warehouse.value}`}
                                                    checked={isChecked}
                                                    onChange={(e) => {
                                                    if (e.target.checked) {
                                                        setWarehouseValue(prev => [...prev, warehouse]);
                                                    } else {
                                                        setWarehouseValue(prev => prev.filter(w => w.value !== warehouse.value));
                                                    }
                                                    }}
                                                />
                                                <div className="control__indicator" />
                                    ...
                                    {errors['warehouses'] && <span className="text-danger">{errors['warehouses']}</span>}
